...
CREATE TABLE IF NOT EXISTS bill (
    ...
    CONSTRAINT date_check CHECK (
        date_of_bill >= (SELECT date_start_contract FROM contract WHERE contract_id = bill.contract_id)
    ),
    CHECK (gross = (SELECT price FROM package WHERE package.id_package = (SELECT package_id FROM contract WHERE contract.id_contract = bill.contract_id)))
);
